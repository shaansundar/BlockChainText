...
client = Client('A3a9yYMkEQbSS5sD33K9Tz')
...
class File():
    ...
    def uploadFile(self):
        filelink = client.upload(filepath="/home/notthatsundar/projects/Text/text_py2/"+fileName+".txt")
        print(filelink.url)
        showinfo(title="Upload Successful", message="Your file is stored in the IPFS server, and the URL is: "+filelink.url+"\n\nMade by : Shaan Sundar (@notthatsundar)")
    def saveFile(self):
        try:
...
def main(root, text, menubar):
    ...
    filemenu.add_command(label="Upload", command=objFile.uploadFile)
